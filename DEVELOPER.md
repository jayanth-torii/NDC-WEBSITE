...
### Students page — Mentoring Cell section rebuild + further editorial redesign pass (this commit)

**`StudentsPage/MentoringCell.tsx`** rebuilt to match a supplied visual reference, replacing the previous plain two-column layout:
- Title now splits its last word into orange (`Mentoring` navy / `Cell` orange), with a small navy-dot-navy underline accent and an eyebrow row (flanking rule lines + an icon badge + "GUIDANCE & SUPPORT").
- New decorative marks flank the heading on large screens: a hand-built `MentorMark` (stacked navy/orange icon badges plus a speech-bubble accent, in the same hand-drawn-SVG spirit as `LibraryIllustration.tsx`) on the left, and a faint outlined `GuidanceMark` (Users + ShieldCheck) on the right. No source illustration asset existed for either, so both are composed from `lucide-react` icons rather than a sourced image.
- "Guidelines For Mentors" card: added a navy side rail with an orange corner accent, an orange icon badge next to the heading, and a dashed vertical connector threading through numbered (`01`–`08`) circular badges, with dashed row dividers between points.
- "Committee Members": header is now a navy pill with an attached orange icon badge (was plain text); each member row gained a hover state, a divider, and a trailing chevron.
- Added a navy/orange SVG wave divider at the section's bottom edge that blends into the next section (`RedressalCell`, which is already navy), matching the reference's footer transition.

The same batch also carries a further round of the ongoing dot-grid/blurred-glow/navy-orange editorial pass (see "Continuing editorial redesign pass" above), reviewed file-by-file before committing:
- **`Activities/CommonComponents/About.tsx`**: hero rebuilt into a two-column layout (copy + a per-club illustration pulled from 11 new `public/images/decor/about_*.png` assets, keyed off the club title, with a placeholder fallback), a new eyebrow + bottom icon feature bar, and bespoke "Objectives"/"Functions" layouts (numbered grid / 3-column icon grid) instead of the old generic list branch.
- **`Activities/CommonComponents/Policies.tsx` / `Procedure.tsx`**: icon-driven tab/point styling throughout; `Procedure.tsx` gained a special-cased dark "grievance procedure" card (detected by regex-matching a `forms.gle` link inside `points`) with its own 4-step layout and CTA — this detection is coupled to specific CMS text content rather than a dedicated data field, worth hardening later if that copy ever changes.
- **`Activities/CulturalLeadershipActivities.tsx`**: largest diff in the batch — objectives/activities/coordinators now auto-chunk into 4-item columns with per-section icon themes and auto-detected "dated" bullet chips, replacing the old fixed-split layout; the static `theatre_masks.png` illustration was dropped. Two tab keys were also renamed ("Ranga Chathanya" → "Ranga Chaithanya", "Rotaract Club" → "Rotaract club") — double-check these match the live CMS/JSON title strings, since a mismatch would silently fall back to the default icon.
- **`Header.tsx`**: scroll-sticky logic now throttles via `requestAnimationFrame` and adds hysteresis (enter sticky past 100px, exit only below 60px) to stop header "shake" during momentum-scroll jitter.
- **`HomePage/Blogs.tsx`, `HomePage/CertificateCourses.tsx`, `IIC/IICMembers.tsx`**: decorative-only polish (blur blobs, dot-grid SVGs, floating rings). `CertificateCourses.tsx` also rebalanced its hero grid (5/7 → 4/8 columns) and removed the `squiggle.png` accent near the CTA with no replacement — confirm that removal was intentional.
- **`QuestionBank/FindQuestionBank.tsx`, `ProgrammeQuestionPapers.tsx`, `QuestionBankTabs.tsx`**: heavier card/icon treatment and hand-built empty-state illustrations throughout. Note two behavioral removals, not just restyles: `FindQuestionBank.tsx` dropped its standalone page header, and `QuestionBankTabs.tsx` removed the sticky tab bar, scroll-to-content-on-click behavior, and the left/right scroll-shadow indicators — confirm dropping those affordances was deliberate rather than lost in the restyle.
- **`globals.css`**: page background changed from a flat fill to a faint repeating dot-grid texture.
