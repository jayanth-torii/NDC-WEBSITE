    ...
    children: [
      {
        ...
        href: "/students",
        children: [
          { label: "Mentoring Cell", href: "/students#mentoring-cell" },
          { label: "Redressal Cell", href: "/students#redressal-cell" },
          { label: "Career Advancement Center", href: "/students#career-advancement" },
          { label: "Training & Placement ", href: "/students#tpi-cell" },
        ]
      },
...
const mobileLinkClass = (active: boolean) =>
  `cursor-pointer text-left text-base font-semibold transition-colors duration-200 focus:outline-none ${active ? "text-orange" : "text-navy hover:text-orange"
  }`;
...
const Header = () => {
  ...
  return (
    <nav
      ref={navRef}
      className={`sticky top-0 z-[50] w-full flex items-center justify-between border-b border-card-border bg-white/95 backdrop-blur-sm pl-0 pr-5 md:px-6 transition-all duration-500 ease-[var(--ease-editorial)] ${isSticky ? "h-[70px] md:h-[76px] py-2 shadow-md" : "h-[85px] md:h-[100px] lg:h-[110px] py-2 md:py-4 lg:py-4"
        }`}
    >
      {/* Logo */}
              ...
              <>
                ...
                {openDesktopDropdown === item.label && (
                  <div
                    ...
                    className="absolute left-1/2 top-full z-50 mt-3 w-56 -translate-x-1/2 overflow-visible rounded-xl border border-card-border bg-white py-2 shadow-[var(--shadow-card-hover)]"
                  >
                    {item.children.map((child) => (
                      <div key={child.label} className="relative group">
                        <button
                          onClick={() => { if (!child.children) go(child.href) }}
                          className={`${dropdownLinkClass} w-full text-left flex justify-between items-center`}
                        >
                          {child.label}
                          {child.children && <IconChevronDown size={14} className="rotate-[-90deg]" />}
                        </button>
                        {child.children && (
                          <div className="absolute left-full top-0 hidden group-hover:block ml-0 w-64 rounded-xl border border-card-border bg-white py-2 shadow-[var(--shadow-card-hover)]">
                            {child.children.map(subChild => (
                              <button
                                key={subChild.label}
                                onClick={() => go(subChild.href)}
                      ...
                      <div key={child.label} className="flex flex-col">
                        {child.children ? (
                          <div className="flex flex-col space-y-2">
                            <button onClick={() => go(child.href)} className={`text-sm font-bold text-navy/70 uppercase tracking-wider text-left`}>{child.label}</button>
                            <div className="flex flex-col space-y-2 pl-3 border-l-2 border-orange/20">
                              {child.children.map(subChild => (
                                <button
                                  key={subChild.label}
                                  onClick={() => go(subChild.href)}
                                  className={mobileLinkClass(pathname === subChild.href)}
                                >
                                  {subChild.label}
                                </button>
                              ))}
