/**
 * students-grievance-redressal-cell controller
 */

import { factories } from '@strapi/strapi'

export default factories.createCoreController('api::students-grievance-redressal-cell.students-grievance-redressal-cell', ({ strapi }) => ({
  async find(ctx) {
    try {
      const entity = await strapi.entityService.findOne('api::students-grievance-redressal-cell.students-grievance-redressal-cell', 1, {
        populate: {
      ...
      const formatEntry = (entry: any) => ({
        ...
        proceduresSection: (entry?.ProceduresSection || []).map((proc: any) => ({
          title: proc?.title || '',
        ...
        Members: {
          title: entry?.Members?.title,
          descriptions : (entry?.Members?.descriptions || []).map((desc: any) => desc?.text || ""),
          tableSection: (entry?.Members?.TableSection || []).map(member => ({
            name: member.name,
      ...
      console.error('Error fetching students-grievance-redressal-cell data:', error);
      return ctx.badRequest('Error fetching students-grievance-redressal-cell data', { moreDetails: error.message });
    }
  },
}));
