/**
 * ambedkar-study-circle controller
 */

import { factories } from '@strapi/strapi'

export default factories.createCoreController('api::ambedkar-study-circle.ambedkar-study-circle', ({ strapi }) => ({
  async find(ctx) {
    try {
      const entity = await strapi.entityService.findOne('api::ambedkar-study-circle.ambedkar-study-circle', 1, {
        populate: {
      ...
      console.error('Error fetching ambedkar-study-circle data:', error);
      return ctx.badRequest('Error fetching ambedkar-study-circle data', { moreDetails: error.message });
    }
  },
}));
